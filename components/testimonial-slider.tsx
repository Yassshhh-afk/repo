    ...
    <div className="relative">
      ...
      <div className="flex items-center justify-center">
        ...
        >
          ...
          <div className="mb-8">
            <div className="relative w-24 h-24 md:w-32 md:h-32 mx-auto">
              <Image
                ...
                width={128}
                height={128}
                className="w-full h-full rounded-full object-cover border-3 border-[#2c365a] shadow-lg transition-transform duration-300"
              />
            </div>
          </div>
          ...
          {/* Testimonial Text */}
          <div className="mb-8">
            <p className="text-lg md:text-xl leading-relaxed text-white italic">"{currentTestimonial.text}"</p>
          </div>

          {/* Client Info */}
          <div className="mb-8">
            <h4 className="text-xl md:text-2xl font-bold text-galaxy-gold mb-1">{currentTestimonial.name}</h4>

            <p className="text-base md:text-lg font-semibold text-white/80">{currentTestimonial.role}</p>
          </div>
        </div>
        ...
        {testimonials.map((_, index) => (
          <button
            ...
            className={`w-3 h-3 rounded-full transition-all duration-300 ${
              index === currentIndex ? "bg-[#2c365a] scale-110" : "bg-black/30 hover:bg-black/50"
            }`}
